...
class FontManager(object):
        ...
        if font_name in self:
            ...
            try:
                if self._use_pk:
                    ...
                    font = self._fonts[
                        font_name] = self._load_mapped_font(font_name)
            except FontNotFound:
                # We try to load a virtual font as a last resort
                font = self._fonts[
                    font_name] = self._load_virtual_font(font_name)
    ...
    def _load_mapped_font(self, tex_font_name):
        ...
        try:
            ...
            self._logger.debug("Font %s is mapped to %s" %
                               (tex_font_name, filename))
            return font_class(self, self._get_new_font_id(), filename)
        except KeyError:
            raise FontNotFound(
                "Could not found a mapped font for %s" % (tex_font_name))
    ...
    def _load_virtual_font(self, tex_font_name):
        ...
        try:
            ...
            raise FontNotFound(
                "Could not found a virtual font for %s" % (tex_font_name))
