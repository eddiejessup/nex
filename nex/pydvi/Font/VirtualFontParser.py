...
class OpcodeParser_fnt_def(OpcodeParser):
    ...
    def read_parameters(self, virtual_font_parser):
        ...
        name = stream.read(stream.read_unsigned_byte1() +
                           stream.read_unsigned_byte1()).decode('ascii')
        ...
        # Fixme: right?
        scale_factor = int(
            to_fix_word(scale_factor) * design_size / virtual_font.design_font_size)
...
#             )
        ...
        self.virtual_font._set_preambule_data(
            file_id, comment, design_font_size, checksum)
