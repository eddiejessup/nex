...
from ..Tools.TexCommentedFile import without_comments
...
class FontMap(object):
    ...
    def __init__(self, filename):
        ...
        # try:
        with open(filename, 'r') as font_map_file:
            for line in without_comments(font_map_file):
                try:
    ...
    def _parse_line(self, line):
        ...
        if first_double_quote_index != -1:
            last_double_quote_index = line.find(
                '"', first_double_quote_index+1)
            ps_snippet = line[
                first_double_quote_index+1:last_double_quote_index]
            effects = FontMap._parse_effects(ps_snippet)
            line = line[:first_double_quote_index] + \
                line[last_double_quote_index+1:]
        else:
            ps_snippet = ''
            effects = {}
        ...
        for filename in filenames:
            if filename.startswith('['):
                ...
                raise ValueError(
                    "Unknown file extension for file %s" % (filename))
