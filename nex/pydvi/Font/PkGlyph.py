...
class PkGlyph(object):
    ...
    def _next_packed_number(self):
        ...
        if i == 0:  # large run count
            # count the number of zeros, coding the number of nybbles and read
            # the first nybble
            while True:
                j = self._next_nybble()
                i += 1
            ...
            return i
        # two-nybble packed number: [dyn_f +1, (13 - dyn_f)*16 + dyn_f]
        elif i < 14:
            next_number = self.dyn_f + 1
            # packed number = (nybble_1 - next_number)*16 + nybble_2 + next_number
            # upper number = (13 - (dyn_f +1))*16 + 15 + (dyn_f +1) = (13 -
            # dyn_f)*16 + dyn_f
    ...
    def _decode_packed_glyph(self):
        ...
        glyph_bitmap = self.glyph_bitmap = np.zeros(
            (self.height, self.width), dtype=np.bool)
        ...
        while y < self.height:
            ...
            while count > 0:
                ...
                # else split and repeat row if necessary
                    ...
                    # copy repeat_row_count times the current row and increment
                    # y
