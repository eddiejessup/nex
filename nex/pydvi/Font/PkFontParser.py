...
from ..TeXUnit import *
from ..Tools.EnumFactory import ExplicitEnumFactory
...
opcode_definitions = (
    ...
    # Fixme: pre is not used in the loop
...
#             )
    ...
    def _read_char_definition(self, flag):
        ...
        if format == 1:
            # (flag mod 4)*256 + pl
            packet_length = (two_least_significant << 8) + \
                stream.read_unsigned_byte1()
            preambule_length = 8  # 3 + 1*5
        elif format == 2:
            packet_length = (two_least_significant << 16) + \
