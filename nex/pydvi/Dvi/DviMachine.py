    ...
    def _run(self, dvi_machine, compute_bounding_box=False):
        ...
        for char_code in self.characters:
            if font.tfm is not None:
                ...
                char_depth = glyph.px_to_mm(
                    glyph.height_px - glyph.horizontal_bearing_y_px)
...
class Opcode_putset_rule(Opcode):
    ...
    def run(self, dvi_machine, compute_bounding_box=False):
        ...
        if compute_bounding_box:
            ...
            dvi_machine.paint_rule(
                registers.h, registers.v, self.width, self.height)
...
class DviProgam(object):
    ...
    def __getitem__(self, i):
        ...
        # Fixme: for long document, the dvi stream should be read on demand:
        # lazy loading
        return self.pages[i]
...
class DviMachine(object):
    ...
    def __init__(self, font_manager):
        ...
        self.virtual_fonts = {}
        # indexed by TeX font id which is not an incremental number starting
        # from 0
        self.fonts = {}
        self._reset()
    ...
    def simplify_dvi_program(self):
        ...
        dvi_simplify_machine.load_dvi_program(
            self.dvi_program, load_fonts=False)
    ...
    def _adjust_opcode_counts_for_virtual_characters(self, opcode_program):
        ...
        for opcode in opcode_program:
            if isinstance(opcode, Opcode_font):
                ...
                self._adjust_opcode_counts(
                    opcode_program, virtual_font, opcode.characters)
    ...
    def run_subroutine(self, subroutine):
        ...
        self._current_font_id = self._virtual_font.font_id_map[
            self._virtual_font.first_font]
    ...
    # !# , glyph
    def paint_char(self, x, y, char_bounding_box, font, dvi_font, char_code):
...
class DviSimplifyMachine(DviMachine):
    ...
    def process_page_xxx_opcodes(self, program_page):
        ...
        _module_logger.info(
            'Process the xxx opcodes in the page program #%u' % program_page.page_number)
        ...
        while i < len(program_page):
            ...
            if isinstance(opcode, Opcode_xxx):
                ...
                if xxx_code.startswith(self.xxx_papersize):
                    new_opcode = self.transform_xxx_paper_size(
                        program_page, xxx_code)
                elif xxx_code == self.xxx_landscape:
                    new_opcode = self.transform_xxx_paper_orientation(
                        program_page, xxx_code)
                elif xxx_code.startswith(self.xxx_colour):
                    new_opcode = self.transform_xxx_colour(
                        program_page, xxx_code)
                else:
                    new_opcode = None
    ...
    def transform_xxx_paper_size(self, program_page, xxx_code):
        ...
        _module_logger.info(
            "Transform the xxx paper size opcode: '%s'" % xxx_code)
    ...
    def transform_xxx_paper_orientation(self, program_page, xxx_code):
        ...
        _module_logger.info(
            "Transform the paper orientation opcode: '%s'" % xxx_code)
    ...
    def simplify_page(self, program_page):
        ...
        _module_logger.info(
            'Simplify the program page #%u' % program_page.page_number)
        ...
        while i < len(program_page):
            ...
            if previous_opcode is not None and opcode.__class__ == previous_opcode.__class__:
                _module_logger.info(
                    "Same opcode %s" % opcode.__class__.__name__)
