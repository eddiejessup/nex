...
class OpcodeParser_xxx(OpcodeParser):
    ...
    def __init__(self, opcode):
        ...
        self.read_unsigned_byten = AbstractStream.read_unsigned_byten[
            self.opcode - self.base_opcode]

    def read_parameters(self, dvi_parser):
        stream = dvi_parser.stream
        return [stream.read(self.read_unsigned_byten(stream)).decode('ascii')]
...
class OpcodeParser_fnt_def(OpcodeParser):
    ...
    def read_parameters(self, dvi_parser):
        ...
        font_name_bytes = stream.read(stream.read_unsigned_byte1() + stream.read_unsigned_byte1())
        font_name = font_name_bytes.decode('ascii')
        font = DviFont(
            font_id, font_name, font_checksum, font_scale_factor, font_design_size)
...
opcode_definitions = (
    ...
    # ... to SET4
    (dvi_opcodes.SET_RULE, 'set rule',
     'typeset a rule and move right', (4, 4), Opcode_set_rule),
    (dvi_opcodes.PUT1, 'put', 'typeset a character', [1, 4], Opcode_put_char),
    # ... to PUT4
    (dvi_opcodes.PUT_RULE, 'put rule',
     'typeset a rule', (4, 4), Opcode_put_rule),
    ...
    (dvi_opcodes.PUSH, 'push',
     'save the current positions', None, Opcode_push),
...
class DviParser(object):
    ...
    def _reset(self):
        ...
        # can be used for lazy loading, reverse if backward
    ...
    def process_stream(self, stream):
        ...
        # Fixme: retrieve bop pointers but don't read the page
    ...
    def _process_postambule(self):
        ...
        self.dvi_program.set_postambule_data(
            max_height, max_width, stack_depth, number_of_pages)
    ...
    def _process_pages_backward(self):
        ...
        while bop_pointer >= 0:
            ...
            self.page_number -= 1
            self._logger.debug(
                'BOP at {}, page # {}'.format(stream.tell(), self.page_number))
    ...
    def process_page(self):
        ...
        # These counters are intended to allocate memory at the beginning of a
        # page rendering.
        font_id = None
        char_counter = {}
        rule_counter = 0
